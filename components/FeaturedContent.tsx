...
const UniversityHostels = ({ hostels, universityName, onViewHostel, savedHostelIds, onToggleSave, hasActiveFilter, initialHostelCount }: UniversityHostelsProps) => {
  ...
  return (
    <div ref={ref} className={`mt-16 transition-all duration-700 delay-100 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <h2 className="text-2xl font-bold text-unistay-navy mb-6">Hostels Near {universityName}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {hostels.map(hostel => (
          // FIX: Wrap HostelCard in a div and move the key here to satisfy React's requirement for keys on list items without passing it as a prop to HostelCard.
          <div key={hostel.id}>
            <HostelCard 
              hostel={hostel} 
